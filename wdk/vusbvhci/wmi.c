...
*/
{
	...
	if(!commonData->IsFDO)
	{
		...
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_WMI, "PDO %s", WMIMinorFunctionString(stack->MinorFunction));
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_WMI, "FDO %s", WMIMinorFunctionString(stack->MinorFunction));
...
skipGuid:;
	...
	case IrpNotWmi:
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_WMI, "IrpNotWmi -- MinorFunction: %lu", (ULONG)stack->MinorFunction);
	case IrpForward:
	...
	default:
		...
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_WMI, "MinorFunction: %lu", (ULONG)stack->MinorFunction);
		ASSERT(FALSE);
		IoSkipCurrentIrpStackLocation(Irp);
		status = IoCallDriver(fdoData->NextLowerDriver, Irp);
		break;
