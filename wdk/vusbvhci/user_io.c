	...
	)
{
	...
	TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_USER_IO, "CtlCode: %s", DbgUserDeviceControlString(irpStack->Parameters.DeviceIoControl.IoControlCode));
	...
	case IOCTL_USB_GET_ROOT_HUB_NAME:
		...
		inf = len;
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_USER_IO, "RootHubSymlink: %ws", ((PUSB_HCD_DRIVERKEY_NAME)Irp->AssociatedIrp.SystemBuffer)->DriverKeyName);
		break;
	...
	case IOCTL_GET_HCD_DRIVERKEY_NAME:
		...
		if(!NT_SUCCESS(status) &&
		   (status != STATUS_BUFFER_TOO_SMALL))
		{
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_USER_IO, "IoGetDeviceProperty failed (%!STATUS!)", status);
			break;
		}
		...
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_USER_IO, "DevicePropertyDriverKeyName: %ws", ((PUSB_HCD_DRIVERKEY_NAME)Irp->AssociatedIrp.SystemBuffer)->DriverKeyName);
		break;

	default:
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_USER_IO, "invalid IOCTL: 0x%08lx", irpStack->Parameters.DeviceIoControl.IoControlCode);
		status = STATUS_INVALID_DEVICE_REQUEST;
		break;
	}
