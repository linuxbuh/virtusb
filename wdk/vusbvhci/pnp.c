...
*/
{
	...
	case IRP_MN_REMOVE_DEVICE:
		...
		if(FdoData->RHubPdo)
		{
			...
			{
				destroyPdo = FALSE;
				TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_PNP, "Root hub pdo was surprise removed: 0x%p", pdoData->Self);
			...
			if(destroyPdo)
			{
				...
				TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_PNP, "Deleting PDO: 0x%p", pdoData->Self);
	...
	case IRP_MN_QUERY_DEVICE_RELATIONS:
		...
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_PNP, "#PDOS present = %lu", (ULONG)!!pdoPresent);
...
*/
{
	...
	case IRP_MN_START_DEVICE:
		...
		if(NT_SUCCESS(status))
		{
			...
			if(!NT_SUCCESS(status))
			{
				TraceEvents(TRACE_LEVEL_ERROR, TRACE_PNP, "IoSetDeviceInterfaceState failed: %!STATUS!", status);
			}
		}
		else
		{
			TraceEvents(TRACE_LEVEL_ERROR, TRACE_PNP, "VUsbVhci_GetAndStoreRootHubName failed: %!STATUS!", status);
		}
...
*/
{
	...
	if(!NT_SUCCESS(status))
	{
		TraceEvents(TRACE_LEVEL_ERROR, TRACE_PNP, "QueryDeviceCaps failed");
		return status;
	}
...
*/
{
	...
	case BusQueryDeviceID:
		...
		TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_PNP, "DeviceId: (%lu)\"%ws\"", length / sizeof(WCHAR) - 1, buffer);
		break;
	...
	case BusQueryHardwareIDs:
...
#if DBG
		{
			...
			do
			{
				...
				if(*start)
				{
					TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_PNP, "HardwareId[%lu]: (%lu)\"%ws\"", cnt, (ULONG)(ULONG_PTR)(buffer - start) - 1, start);
					cnt++;
				}
			} while(*buffer);
	...
	case DeviceTextDescription:
		...
		case 0x00000409: // English
			if(!Irp->IoStatus.Information)
			{
				...
				ASSERT(!buffer[length / sizeof(WCHAR) - 1]);
				Irp->IoStatus.Information = (ULONG_PTR)buffer;
				TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_PNP, "Text: (%lu)\"%ws\"", length / sizeof(WCHAR) - 1, buffer);
			}
			status = STATUS_SUCCESS;
			break;
		}
