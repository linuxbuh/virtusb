...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_DispatchPnP\n");
	...
	if(commonData->IsFDO)
	{
		VIRTUSB_KDPRINT2("FDO %s IRP: 0x%p\n",
	...
	{
		VIRTUSB_KDPRINT2("PDO %s IRP: 0x%p\n",
		                 PnPMinorFunctionString(irpStack->MinorFunction),
		                 Irp);
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_FDO_PnP\n");
	...
	case IRP_MN_REMOVE_DEVICE:
		    ...
		    entry = nextEntry, nextEntry = entry->Flink)
		{
			...
			{
				VIRTUSB_KDPRINT1("Found a surprise removed pdo: 0x%p\n", pdoData->Self);
				InitializeListHead(entry);
			...
			VIRTUSB_KDPRINT2("Deleting PDO: 0x%p  ID: %lu\n", pdoData->Self, pdoData->Id);
		...
		VIRTUSB_KDPRINT1("Deleting FDO: 0x%p\n", fdo);
		IoDeleteDevice(fdo);

		return status;

	case IRP_MN_QUERY_DEVICE_RELATIONS:
		VIRTUSB_KDPRINT1("QueryDeviceRelation Type: %s\n",
		...
		VIRTUSB_KDPRINT1("#PDOS present = %lu\n", numPdosPresent);
		VIRTUSB_KDPRINT1("#PDOs reported = %lu\n", relations->Count);
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_PDO_PnP\n");
	...
	case IRP_MN_QUERY_ID:
		...
		VIRTUSB_KDPRINT1("QueryId Type: %s\n",
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_PDO_QueryDeviceCaps\n");
	...
	if(!NT_SUCCESS(status))
	{
		VIRTUSB_KDPRINT("QueryDeviceCaps failed\n");
		return status;
	}
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_PDO_QueryDeviceId\n");
	...
	case BusQueryDeviceID:
		...
		VIRTUSB_KDPRINT2("DeviceId: (%lu)\"%ws\"\n", length / sizeof(WCHAR) - 1, buffer);
		break;
	...
	case BusQueryInstanceID:
		...
		ASSERT(!buffer[length / sizeof(WCHAR) - 1]);
		Irp->IoStatus.Information = (ULONG_PTR)buffer;
		VIRTUSB_KDPRINT2("InstanceId: (%lu)\"%ws\"\n", length / sizeof(WCHAR) - 1, buffer);
		break;
	...
	case BusQueryHardwareIDs:
...
#if DBG
		{
			...
			do
			{
				...
				if(*start)
				{
					VIRTUSB_KDPRINT3("HardwareId[%lu]: (%lu)\"%ws\"\n", cnt, (ULONG)(ULONG_PTR)(buffer - start) - 1, start);
					cnt++;
				}
			} while(*buffer);
			VIRTUSB_KDPRINT1("#HardwareIds: %lu\n", cnt);
		}
#endif
		break;
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_PDO_QueryDeviceText\n");
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_PDO_QueryResources\n");
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_PDO_QueryResourceRequirements\n");
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_PDO_QueryDeviceRelations\n");
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_PDO_QueryBusInformation\n");
	...
	)
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_PDO_QueryInterface\n");
...
*/
{
	...
	VIRTUSB_KDPRINT("in VirtUsb_GetDeviceCapabilities\n");
