...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_DispatchPower\n");
	...
	if(commonData->IsFDO)
	{
		VUSBVHCI_KDPRINT4("FDO %s IRP: 0x%p %s %s\n",
	...
	{
		VUSBVHCI_KDPRINT4("PDO %s IRP: 0x%p %s %s\n",
		                  PowerMinorFunctionString(irpStack->MinorFunction),
		                  Irp,
		                  DbgSystemPowerString(commonData->SystemPowerState),
		                  DbgDevicePowerString(commonData->DevicePowerState));
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_FDO_Power\n");
	...
	{
		VUSBVHCI_KDPRINT2("Request to set %s state to %s\n",
...

{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_PDO_Power\n");
	...
	{
	case IRP_MN_SET_POWER:
		VUSBVHCI_KDPRINT2("Setting %s power state to %s\n",
		                  ((powerType == SystemPowerState) ?  "System" : "Device"),
		                  ((powerType == SystemPowerState) ?
		                  DbgSystemPowerString(powerState.SystemState) :
		                  DbgDevicePowerString(powerState.DeviceState)));
