...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_DispatchInternalDeviceControl\n");
	...
	VUSBVHCI_KDPRINT1("CtlCode: %s\n", DbgInternalDeviceControlString(irpStack->Parameters.DeviceIoControl.IoControlCode));
...
*/
				VUSBVHCI_KDPRINT("someone used two arguments on IOCTL_INTERNAL_USB_GET_DEVICE_HANDLE\n");
				// TODO: implement
				KdBreakPoint();
			...
			else
			{
				*(PUSB_DEVICE_HANDLE *)irpStack->Parameters.Others.Argument1 =
				...
				VUSBVHCI_KDPRINT1("RootHub device handle: 0x%p\n", *(PUSB_DEVICE_HANDLE *)irpStack->Parameters.Others.Argument1);
			}
		}
		break;
#endif
	...
	default:
		VUSBVHCI_KDPRINT1("invalid internal IOCTL: 0x%08lx\n", irpStack->Parameters.DeviceIoControl.IoControlCode);
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_ProcUrb\n");
	...
	{
		VUSBVHCI_KDPRINT("'very' invalid URB\n");
	...
	if(!rhub)
	{
		VUSBVHCI_KDPRINT("parent interface down\n");
	...
	{
		VUSBVHCI_KDPRINT("root-hub got an URB\n");
		// in case it is zero
		urb->UrbHeader.UsbdDeviceHandle = rhub;
	}
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_NormalizeUrb\n");
...
CheckBuffer:
	...
	default:
		VUSBVHCI_KDPRINT1("unknown urb function: %hu\n", Urb->UrbHeader.Function);
