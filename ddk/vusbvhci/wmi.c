...
#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, VUsbVhci_DispatchSystemControl)
#pragma alloc_text(PAGE, VUsbVhci_WmiRegistration)
#pragma alloc_text(PAGE, VUsbVhci_WmiDeRegistration)
#pragma alloc_text(PAGE, VUsbVhci_SetWmiDataItem)
#pragma alloc_text(PAGE, VUsbVhci_SetWmiDataBlock)
#pragma alloc_text(PAGE, VUsbVhci_QueryWmiDataBlock)
#pragma alloc_text(PAGE, VUsbVhci_QueryWmiRegInfo)
#endif // ALLOC_PRAGMA
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_DispatchSystemControl\n");
	...
	if(stack->Parameters.WMI.DataPath)
	{
		...
		VUSBVHCI_KDPRINT1("GUID: %wZ\n", &ustr);
		ExFreePoolWithTag(ustr.Buffer, VUSBVHCI_POOL_TAG);
skipGuid:;
	}
#endif
	...
	case IrpNotWmi:
		VUSBVHCI_KDPRINT1("IrpNotWmi -- MinorFunction: %lu\n", (ULONG)stack->MinorFunction);
	case IrpForward:
	...
	default:
		...
		VUSBVHCI_KDPRINT1("MinorFunction: %lu\n", (ULONG)stack->MinorFunction);
		ASSERT(FALSE);
		IoSkipCurrentIrpStackLocation(Irp);
		status = IoCallDriver(fdoData->NextLowerDriver, Irp);
		break;
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_WmiRegistration\n");

	FdoData->WmiLibInfo.GuidCount = 0;
	FdoData->WmiLibInfo.GuidList = NULL;
	FdoData->WmiLibInfo.QueryWmiRegInfo = VUsbVhci_QueryWmiRegInfo;
	FdoData->WmiLibInfo.QueryWmiDataBlock = VUsbVhci_QueryWmiDataBlock;
	FdoData->WmiLibInfo.SetWmiDataBlock = VUsbVhci_SetWmiDataBlock;
	FdoData->WmiLibInfo.SetWmiDataItem = VUsbVhci_SetWmiDataItem;
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_WmiDeRegistration\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_SetWmiDataItem\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_SetWmiDataBlock\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_QueryWmiDataBlock\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_QueryWmiRegInfo\n");
