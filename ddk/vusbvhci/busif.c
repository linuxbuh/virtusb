	...
	)
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_InterfaceReference\n");
	ASSERT(rhub);
	...
	)
{
	...
	ULONG            res;
	ASSERT(KeGetCurrentIrql() <= DISPATCH_LEVEL);
	VUSBVHCI_KDPRINT("in VUsbVhci_InterfaceDereference\n");
	ASSERT(rhub);
	pdoData = ((PDEVICE_OBJECT)rhub->HcdContext)->DeviceExtension;
	ASSERT(pdoData);
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_CreateUsbDevice\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_InitializeUsbDevice\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_RemoveUsbDevice\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_GetUsbDescriptors\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_RestoreDevice\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_GetUsbDeviceHackFlags\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_GetUsbPortHackFlags\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_QueryDeviceInformation\n");

	VUSBVHCI_KDPRINT1("--> BusContext:                    0x%p\n", BusContext);
	VUSBVHCI_KDPRINT1("--> DeviceHandle:                  0x%p\n", DeviceHandle);
	VUSBVHCI_KDPRINT1("--> DeviceInformationBuffer:       0x%p\n", DeviceInformationBuffer);
	VUSBVHCI_KDPRINT1("--> DeviceInformationBufferLength: %lu\n",  DeviceInformationBufferLength);
	VUSBVHCI_KDPRINT1("--> LengthOfDataReturned:          0x%p\n", LengthOfDataReturned);
	...
	if(dev->State != Addressed && dev->State != Configured)
	{
		VUSBVHCI_KDPRINT("device is not addressed or configured\n");
	...
	{
		VUSBVHCI_KDPRINT("buffer too small -- even to report required buffer size\n");
	...
	{
		VUSBVHCI_KDPRINT("InformationLevel != 0\n");
		VUSBVHCI_KDPRINT1("InformationLevel == %lu\n", inf->InformationLevel);
	...
	{
		VUSBVHCI_KDPRINT("buffer too small -- even without endpoints\n");
		goto BufferTooSmall;
	}
	...
	if(dev->State == Configured)
	{
		...
		for(i = 0; i < conf->InterfaceCount; i++)
		{
			...
			if(DeviceInformationBufferLength < len)
			{
				VUSBVHCI_KDPRINT("buffer too small\n");
	...
	VUSBVHCI_KDPRINT1("--> *LengthOfDataReturned:         %lu\n", len);
	status = STATUS_SUCCESS;
End:
	VUsbVhci_DereferenceUsbDevice(fdoData, dev);
	return status;
...
BufferTooSmall:
	if(dev->State != Configured)
	{
		...
		VUSBVHCI_KDPRINT1("required buffer size: %lu\n", inf->ActualLength);
		status = STATUS_BUFFER_TOO_SMALL;
		goto End;
	}
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_GetControllerInformation\n");

	VUSBVHCI_KDPRINT1("--> BusContext:                        0x%p\n", BusContext);
	VUSBVHCI_KDPRINT1("--> ControllerInformationBuffer:       0x%p\n", ControllerInformationBuffer);
	VUSBVHCI_KDPRINT1("--> ControllerInformationBufferLength: %lu\n",  ControllerInformationBufferLength);
	VUSBVHCI_KDPRINT1("--> LengthOfDataReturned:              0x%p\n", LengthOfDataReturned);
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_ControllerSelectiveSuspend\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_GetExtendedHubInformation\n");

	VUSBVHCI_KDPRINT1("--> BusContext:                 0x%p\n", BusContext);
	VUSBVHCI_KDPRINT1("--> HubPhysicalDeviceObject:    0x%p\n", HubPhysicalDeviceObject);
	VUSBVHCI_KDPRINT1("--> HubInformationBuffer:       0x%p\n", HubInformationBuffer);
	VUSBVHCI_KDPRINT1("--> HubInformationBufferLength: %lu\n",  HubInformationBufferLength);
	VUSBVHCI_KDPRINT1("--> LengthOfDataReturned:       0x%p\n", LengthOfDataReturned);
	...
	{
		VUSBVHCI_KDPRINT("buffer too small -- even without ports\n");
		return STATUS_BUFFER_TOO_SMALL;
	}
	...
	{
		VUSBVHCI_KDPRINT("GetExtendedHubInformation is not supported for devices other than the root-hub.\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_GetRootHubSymbolicName\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_GetDeviceBusContext\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_Initialize20Hub\n");

	VUSBVHCI_KDPRINT1("HubBusContext   = 0x%p\n", HubBusContext);
	VUSBVHCI_KDPRINT1("HubDeviceHandle = 0x%p\n", HubDeviceHandle);
	VUSBVHCI_KDPRINT1("TtCount         = %lu\n", TtCount);
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_RootHubInitNotification\n");
	...
	if(CallbackFunction)
	{
		VUSBVHCI_KDPRINT("*** NOTIFY ROOT-HUB ***\n");
		// We are ready! -- Come get some!
		CallbackFunction(CallbackContext);
		VUSBVHCI_KDPRINT("*** NOTIFY ROOT-HUB DONE ***\n");
	}
	//else
	//{
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_FlushTransfers\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_SetDeviceHandleData\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_SubmitIsoOutUrb\n");
	return STATUS_NOT_SUPPORTED;
}
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_GetUSBDIVersion\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_QueryBusTime\n");
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_EnumLogEntry\n");
	return STATUS_SUCCESS;
}
...
*/
{
	...
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_QueryBusInformation\n");
	...
	VUSBVHCI_KDPRINT1("Level                       = %lu\n", Level);
	VUSBVHCI_KDPRINT1("BusInformationBuffer        = 0x%p\n", BusInformationBuffer);
	VUSBVHCI_KDPRINT1("BusInformationBufferLength  = 0x%p\n", BusInformationBufferLength);
	VUSBVHCI_KDPRINT1("BusInformationActualLength  = 0x%p\n", BusInformationActualLength);
	...
	VUSBVHCI_KDPRINT1("*BusInformationBufferLength = %lu\n", *BusInformationBufferLength);
	...
	if(dstCharAvail)
	{
		...
		VUSBVHCI_KDPRINT1("HcdSymlink: %ws\n", inf->ControllerNameUnicodeString);
	}
	...
	*BusInformationBufferLength = FIELD_OFFSET(USB_BUS_INFORMATION_LEVEL_1, ControllerNameUnicodeString[0]) +
	...
	VUSBVHCI_KDPRINT1("*BusInformationBufferLength = %lu\n", *BusInformationBufferLength);
	if(BusInformationActualLength)
		VUSBVHCI_KDPRINT1("*BusInformationActualLength = %lu\n", *BusInformationActualLength);
...
{
	PUSBHUB_CONTEXT rhub = BusContext;
	VUSBVHCI_KDPRINT("in VUsbVhci_BUSIF_IsDeviceHighSpeed\n");
